...
from CAxis_to_Sine import convert_CAxis_to_Sine, convert_CAxis_to_Sine_test, get_transform, get_sine_train_test_data
...
if __name__ == '__main__':
    ...
    sine_train, CAxis_train, sine_test, CAxis_test = get_sine_train_test_data(file_path, True, int((1.0-test_per)*line_cnt), batch_size)
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for i in range(0, dataset_size):
            sine_batch_stats = sine_train[i][0]
            CAxis_number = sine_train[i][1]
            CAxis_batch_stats = CAxis_train[CAxis_number-1]


            # reshape tensor for input into generator
            current_sine = torch.reshape(sine_batch_stats, (sine_batch_stats.shape[1], 1, 36))

            current_CAxis = torch.reshape(CAxis_batch_stats, (CAxis_batch_stats.shape[1], 1, 3)).to(device)
            ...
            batch_size = CAxis_batch_stats.shape[1]
    ...
    if (test):
        """Start Testing"""
        #open output files
        real_file = open("./result_files/real.txt", "w")
        fake_file = open("./result_files/fake.txt", "w")

        #write headers to output files
        real_file.write("CAxisLocation_0,CAxisLocation_1,CAxisLocation_2\n")
        fake_file.write("CAxisLocation_0,CAxisLocation_1,CAxisLocation_2\n")


        print("Starting Testing ...")
        for i in range(0, len(sine_test)):
            print("Printing output file "+str(i)+"/"+str(len(sine_test)))
            current_sine = torch.reshape(sine_test[i][0], (sine_test[i][0].shape[1], 1, 36))
            ...
            for i in range(0, fake.shape[0]):
                fake_file.write(str(fake[i,0,0].item())+","+str(fake[i,0,1].item())+","+str(fake[i,0,2].item())+"\n")
                real_file.write(str(current_CAxis[i,0,0].item())+","+str(current_CAxis[i,0,1].item())+","+str(current_CAxis[i,0,2].item())+"\n")
